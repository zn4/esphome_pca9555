...
class PCA9555Component : public Component, public i2c::I2CDevice {
 ...
 protected:
  ...
  bool write_register_(uint8_t reg_start, uint8_t (&value)[2]);


  /// Mask for the pin config - 1 means OUTPUT, 0 means INPUT
  uint8_t config_mask_[2]{0x00, 0x00};
  /// The mask to write as output state - 1 means HIGH, 0 means LOW
  uint8_t output_mask_[2]{0x00, 0x00};
  /// The state of the actual input pin states - 1 means HIGH, 0 means LOW
  uint8_t input_mask_[2]{0x00, 0x00};
  /// Storage for last I2C error seen
  esphome::i2c::ErrorCode last_error_;
};

/// Helper class to expose a PCA9555 pin as an internal input GPIO pin.
